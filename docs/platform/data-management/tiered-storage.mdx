...
- [Configure cloud storage](../../data-management/tiered-storage/#configure-cloud-storage).
- Enable Tiered Storage, either [for a cluster](../../data-management/tiered-storage/#enable-tiered-storage-for-a-cluster) or [for a topic](../../data-management/tiered-storage/#enable-tiered-storage-for-a-topic).
- [Set retention limits](../../data-management/tiered-storage/#set-retention-limits).
...
To configure access to an Azure Blob Storage container with Active Directory authorization: 

Policies are stored in Active Directory that map to ABS. This requires providing an OAuth token. The user provides a public identifier and a secret, which Redpanda exchanges for an OAuth token. This token is then appended to all subsequent requests to ABS.

xxx

Alternatively, to configure access to Azure Blob Storage with shared key credentials:
