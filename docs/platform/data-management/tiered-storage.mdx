...
2. Enable Tiered Storage, either [for a cluster](../../data-management/tiered-storage/#enable-tiered-storage-for-a-cluster) or [for a topic](../../data-management/tiered-storage/#enable-tiered-storage-for-a-topic).
...
To configure access to Google Cloud Storage with access keys instead of IAM roles:
...
To configure access to an Azure Blob Storage container with an IAM role: 
...
2. Run the `rpk cluster config edit` command, then edit the following required properties:
   ...
   cloud_storage_region: eu-north-1
   cloud_storage_azure_container: pandacontainer
   ```

To configure access to Azure Blob Storage with shared keys instead of IAM roles:
