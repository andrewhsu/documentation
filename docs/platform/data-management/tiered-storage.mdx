...
Redpanda supports Tiered Storage with Amazon S3, Google Cloud Storage (GCS), and Microsoft Azure Blob Storage (ABS). 
...
1. [Configure cloud storage](../../data-management/tiered-storage/#configure-cloud-storage).
2. [Enable Tiered Storage for a cluster](../../data-management/tiered-storage/#enable-tiered-storage-for-a-cluster), or [enable Tiered Storage for a topic](../../data-management/tiered-storage/#enable-tiered-storage-for-a-topic).
3. [Set retention limits](../../data-management/tiered-storage/#set-retention-limits).
...
2. Run the `rpk cluster config edit` command, then edit the following required properties:
   ...
   cloud_storage_bucket: <redpanda_bucket>
   ```
...
Configure access to Azure Blob Storage with either shared keys or with Active Directory (using either secrets or managed identities). 

To configure access to Azure Blob Storage with shared keys:

1. Grant a user the following permissions to read and create block blobs on the container to be used with the cluster (or on all containers): `Get Blob`, `Delete Blob`, `Put Blob`.
2. For shared key authorization, copy the `cloud_storage_azure_shared_key` cluster property. For the client ID and secret for Active Directory authorization, copy `cloud_storage_azure_client_id` and `cloud_storage_azure_client_secret`.
3. Run the `rpk cluster config edit` command, then edit the following required properties:
   ...
   cloud_storage_region: <eu-north-1>
   cloud_storage_azure_container: <redpanda_container>
   ```
   
To configure access to an Azure Blob Storage container with Active Directory instead of shared keys: 

*DRAFT*

With managed identities, user doesn't provide secret; instead, user pins the permissions to the virtual machine, and Redpanda fetches them at runtime; see [here](https://learn.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/how-to-use-vm-token). Azure has predefined roles that specify the operations allowed on a certain resource. The flow is to create a managed identity for a VM and then asign an RBAC role to it; see [here](https://learn.microsoft.com/en-us/azure/storage/blobs/authorize-managed-identity?source=recommendations). Redpanda needs to perform the same operations in Azure as in S3, so the most restrictive role we can use is "Storage Account Contributor" applied to the container.

1. Run the `rpk cluster config edit` command, then edit the following required properties:
...
- If properties are not specified:
  - Starting in release 22.3, new topics use the default retention values of 1 (local) and 7 days (cloud).
...
Setting idle timeout to a very short interval can create a lot of small files, which can affect throughput. If you decide to set a value for idle timeout, start out with 600 seconds, which prevents the creation of so many small files that throughput is affected when you recover the files.
...
Reconciliation is a Redpanda process that runs periodically on every node. It monitors partitions and decides which partitions are uploaded on each Redpanda node to guarantee that data is uploaded only once. It also balances the workload evenly between nodes.
...
The reconciliation process is configured with the `cloud_storage_reconciliation_interval_ms` property. Edit the property with the `rpk cluster config edit` command.
...
If the cloud provider replies with an error message that the server is busy, Redpanda retries the request. Redpanda always uses exponential backoff with cloud connections. Redpanda retries the request if it receives any of the following errors:
...
| `cloud_storage_reconciliation_interval_ms` | The interval, in milliseconds, to reconcile partitions that need to be uploaded. A long reconciliation interval can result in a delayed reaction to topic creation, topic deletion, or leadership rebalancing events. A short reconciliation interval guarantees that new partitions are picked up quickly, but the process uses more resources. Default is 10000 ms. |
