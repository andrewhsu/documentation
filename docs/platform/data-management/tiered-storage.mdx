...
2. Copy an account access key for the Azure container you want Redpanda to use. You'll enter this for the `cloud_storage_azure_shared_key` property. For information on how to view your account access keys, see the [Azure documentation](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-keys-manage?toc=%2Fazure%2Fstorage%2Fblobs%2Ftoc.json&bc=%2Fazure%2Fstorage%2Fblobs%2Fbreadcrumb%2Ftoc.json&tabs=azure-portal#view-account-access-keys). 
3. Run the `rpk cluster config edit` command, then edit the following required properties:
...
Reconciliation is a Redpanda process that runs periodically on every node. It monitors partitions and decides which partitions are uploaded on each node to guarantee that data is uploaded only once. It also balances the workload evenly between nodes. 

Redpanda reacts to partition leadership changes. When leadership rebalancing occurs, the reconciliation process could start uploading to cloud storage from a different node.
