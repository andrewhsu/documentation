...
1. Use a [self-signed certificate](#use-a-self-signed-certificate) or a [custom Issuer](#use-a-custom-issuer)
...
- [External access](../../manage-kubernetes/networking-and-connectivity) to your Redpanda cluster. Your Redpanda broker should advertise an address in your domain.
...
CLUSTER
=======
redpanda.a8607359-0459-477a-ac2a-639cc6d3521d
...
1. Use that root CA to validate your connection to Redpanda:

  ```bash
  rpk cluster info --brokers <custom-domain> --tls-truststore ca.crt
  ```

## Use a custom Issuer

You can use your own Issuer or ClusterIssuer to take advantage of an ACME certificate provider such as Let's Encrypt or to use an intermediate certificate authority (CA).
