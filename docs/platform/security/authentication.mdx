...
Simple Authentication Security Layer (SASL) is an authentication framework that lets the user choose the authentication mechanism. Redpanda supports Kerberos (GSSAPI) and the Salted Challenge Response Authentication Mechanism (SCRAM) authentication methods.
...
1. Redpanda administrator verifies that the redpanda user can find, read, and renew the kerberos ticket. Example command to validate correctness and usability of Kerberos keytabs:
