...
```
      ...
      <TabItem value="RHEL/CentOS" label="RHEL/CentOS" default>
...
3. Install required libraries for Kerberos on Redpanda nodes. Configure the `/etc/krb5.conf` file to use Active Directory or other corporate key distribution center (KDC). In the following example, `krb5.conf` has configurations in bold showing realm information that needs to point to the proper corporate KDC.
...
4. For each node, create two principals: `host/<FQDN>@<REALM>.COM` and `redpanda/<FQDN>@<REALM>.COM`, ensuring that the expiration date is suitably long and the tickets are renewable.
...
3. Enable Kerberos (SASL/GSSAPI) for the Kafka API by adding GSSAPI to the Redpanda property `sasl.enabled.mechanisms`:
  ...
  # Update the sasl mechanisms list to include Kerberos (GSSAPI)
...
1. Redpanda administrator generates keytabs for the new node. The following example shows options to regenerate keytabs in the cluster taking into account principals may require unique passwords per corporate policy.
...
2. Redpanda administrator verifies that the redpanda user can find, read, and renew the kerberos ticket. Example command to validate correctness and usability of Kerberos keytabs:
...
#### Enable Kerberos
To enable Kerberos (GSSAPI) authentication, add GSSAPI to the `sasl_mechanisms` list.
...
#### Disable Kerberos
To disable Kerberos authentication, remove GSSAPI from the `sasl_mechanisms` list.
