...
| `redpanda.remote.delete` | Deletes a topic and all topic data, including data in remote storage. This requires that both remote read and remote write are enabled, and the topic must not be a Remote Read Replica topic. To delete a topic but retain data in remote storage, set to `false` before deleting the topic.<br/><br/>Default is `true` for new topics, including new topics on upgraded clusters. Default is `false` for topics created prior to release 22.3. |
...
:::note
Starting in release 22.3, when you delete a topic in Redpanda, the data is also deleted in cloud storage. Topics created prior to release 22.3 can alter their configuration to this behavior on a topic level after upgrading. See [Enable Tiered Storage for a topic](../../manage/tiered-storage/#enable-tiered-storage-for-a-topic)
:::
